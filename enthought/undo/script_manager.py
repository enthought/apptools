...
class _ScriptTraitGet(_ScriptCall):
    ...
    def as_str(self, sm, so_needed):
        ...
        if self.result is None:
            ...
            nr, _ = sm.results[id(self.result)]
...
class ScriptManager(HasTraits):
    """ The ScriptManager class manages the recording of scripts on behalf of
    an undo manager.
    """
    ...
    # recorded script if none is currently being recorded).
    ...
    def _object_as_string(self, obj):
        ...
        nr, _ = self.results.get(obj_id, (None, None))
    ...
    def _save_result(self, result):
        ...
        if id(result) not in self.results:
            self.results[id(result)] = (-1, result)
