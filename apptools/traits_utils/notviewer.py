...
#: Template for the text displayed for notification change events.
CHANGE_TEMPLATE = (
    u"{class_name}.{name}: {old!r} \N{RIGHTWARDS ARROW} {new!r}")

#: Template for the text displayed for calls to notification handlers.
...
def records_to_tree(records, idx):
    """ Transform a list of records in a tree (list-of-lists) recursively.
    """
...
class Thread(HasStrictTraits):
    ...
    def __init__(self, thread_name, record_container):
        ...
        while idx < n_records:
            ...
            event, idx = records_to_tree(records, idx)
            # Eliminate events that are generated by the NotViewer itself.
...
class NotViewer(HasStrictTraits):
    """
    The Traits Notification Viewer.
    """
    ...
    def update_view(self):
        """ Update the tree view of the Traits notification events. """
