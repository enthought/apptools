...
class Updater(HasPrivateTraits):
    ...
    def add_state_function_for_class(self, klass, target_version, function):
        ...
        self.add_state_function(
            klass.__module__, klass.__name__, target_version, function
        )
    ...
    def get_version_attribute(self, module, name):
        ...
        """
        return self.version_attribute_map.get(
            (module, name), self._default_version_attribute
        )
    ...
    def _state_functions_changed(self, old, new):
        logger.debug(
            "Detected state_functions changed from [%s] to [%s] " + "in [%s]",
            old,
            new,
            self,
        )
    ...
    def _state_functions_items_changed(self, event):
        ...
        for k, v in event.removed.items():
            ...
            if count < 0:
                logger.warn(
                    "Unexpectedly reached negative reference count "
                    + "value of [%s] for [%s]",
                    count,
                    key,
                )
        ...
        for k, v in event.changed.items():
            ...
            logger.debug(
                "Detected [%s] changed in state_functions from "
                + "[%s] to [%s] in [%s]",
                k,
                v,
                value,
        ...
        for k, v in event.added.items():
            logger.debug(
                "Detected mapping of [%s] to [%s] added to "
                + "state_functions in [%s]",
                k,
                v,
                self,
            )
