...
class UpdaterTestCase(unittest.TestCase):
    ...
    def test_add_state_function(self):
        ...
        self._validate_state_function_contents(
            [(key, [fn, fn2]), (key2, [fn2])], {(key[0], key[1]): 3}
        )
    ...
    def test_merge_updater(self):
        ...
        counts = {("foo", "Foo"): 1}
        self._validate_state_function_contents(
            list(updater.state_functions.items()), counts
        )
    ...
    def _validate_class_map_contents(self, items):
        ...
        for key, value in items:
            self.assertEqual(
                True,
                key in self.registry.class_map,
                "Key " + str(key) + " not in class_map",
            )
            self.assertEqual(
                value,
                self.registry.class_map[key],
                str(value)
                + " != "
                + str(self.registry.class_map[key])
                + " for key "
                + str(key),
            )
            self.assertEqual(
    ...
    def _validate_state_function_contents(self, items, counts):
        ...
        for key, value in items:
            self.assertEqual(
                True,
                key in self.registry.state_functions,
                "Key " + str(key) + " not in state functions",
            )
            self.assertEqual(
                value,
                self.registry.state_functions[key],
                str(value)
                + " != "
                + str(self.registry.state_functions[key])
                + " for key "
                + str(key),
            )
            self.assertEqual(
                True,
                self.registry.has_state_function(key[0], key[1]),
